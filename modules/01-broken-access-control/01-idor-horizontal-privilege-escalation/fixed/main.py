...
def get_account(account_id: int, current_user: sqlite3.Row = Depends(get_current_user)):
    # Object-level authorization check: kimlik doğrulama (authentication) tek başına
    # yeterli değil — istekte bulunan kullanıcının SADECE kendi kaydına erişebildiğini
    # de doğruluyoruz. Session'daki gerçek sahip (current_user["id"]) ile URL'den gelen
    # account_id eşleşmiyorsa erişim reddedilir. Bu satır olmadan endpoint IDOR/BOLA açığıdır.
    if account_id != current_user["id"]:
        # 403 (404 değil): ID'ler AUTOINCREMENT ile tahmin edilebilir olduğundan kaynağın
        # varlığını 404 ile gizlemenin güvenlik değeri yok; "yetkin yok" mesajı daha dürüst
        # ve API tüketicisi için anlamsal olarak nettir.
